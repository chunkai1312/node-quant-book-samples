...
export class TaifexScraperService {
  ...
  async fetchTxoPutCallRatio(options?: { date: string }) {
    ...
    return { date, txoPutCallRatio: numeral(row[6]).divide(100).value() };
  }
  ...
  async fetchExchangeRates(options?: { date: string }) {
    ...
    return {
      date,
      usdtwd: numeral(row[1]).value(),
      cnytwd: numeral(row[2]).value(),
      eurusd: numeral(row[3]).value(),
      usdjpy: numeral(row[4]).value(),
      gbpusd: numeral(row[5]).value(),
      audusd: numeral(row[6]).value(),
      usdhkd: numeral(row[7]).value(),
      usdcny: numeral(row[8]).value(),
      usdzar: numeral(row[9]).value(),
      nzdusd: numeral(row[10]).value(),
    };
  }
}
